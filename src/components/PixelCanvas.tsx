...
export function PixelCanvas({ document, revision, onChange, onPickColor }: Props) {
  ...
  }, [cursor, document, gridVisible, revision, zoom]);
  ...
  const applyCell = (x: number, y: number) => {
    if (!document.inBounds(x, y)) return;
    const key = `${x}:${y}`;
  ...
  return (
    <canvas
      ...
      onPointerMove={(event) => {
        const { x, y } = cellFromEvent(event);
        if (document.inBounds(x, y)) setCursor({ x, y });
        else setCursor(null);
        if (drawing.current && (tool === "pencil" || tool === "eraser")) applyCell(x, y);
      }}
